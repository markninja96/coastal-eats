...
const adminConfig: DashboardConfig = {
  header: {
    ...
    subtitle:
      'Balance staffing, approve policies, and keep compliance healthy across every region.',
    actions: (
      <>
  ...
  alert: {
    ...
    description:
      'Two additional assignments will trigger consecutive day warnings for Seattle staff.',
    actions: (
      <>
  ...
  primary: {
    ...
    action: { label: 'View shifts', variant: 'secondary', size: 'sm' },
    body: (
      <>
        {SITE_LIST.map((site) => (
          <div
  ...
  secondary: {
    eyebrow: 'Overtime watch',
    title: 'Projected overtime costs',
    description:
      'Three staff are projected above 35 hours if weekend shifts are confirmed.',
    body: (
      <>
...
const managerConfig: DashboardConfig = {
  header: {
    ...
    subtitle:
      'Publish weekly coverage, resolve swaps, and keep staffing levels balanced.',
    actions: (
      <>
  ...
  alert: {
    ...
    description:
      'Assign certified staff or broadcast open roles before Wednesday noon.',
    actions: (
      <>
        <Button variant="outline" size="sm">
          View open shifts
  ...
  primary: {
    eyebrow: 'Coverage risk',
    title: 'Friday gaps',
    action: { label: 'Assign staff', variant: 'secondary', size: 'sm' },
    body: (
      <>
        {SITE_LIST.map((site) => (
          <div
  ...
  secondary: {
    eyebrow: 'On deck',
    title: 'Swap requests',
    description: 'Prioritize swaps that impact published shifts this weekend.',
    body: (
      <>
...
const staffConfig: DashboardConfig = {
  header: {
    eyebrow: 'ShiftSync',
  ...
  primary: {
    ...
    action: { label: 'View schedule', variant: 'secondary', size: 'sm' },
    body: (
      <>
  ...
  secondary: {
    eyebrow: 'Requests',
    title: 'Your activity',
    description: 'Track swaps and availability reminders for this week.',
    body: (
      <>
...
function DashboardLayout({ config }: { config: DashboardConfig }) {
  return (
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.2em] text-ink/40">
                {config.primary.eyebrow}
              </p>
              <h2 className="font-display text-2xl">{config.primary.title}</h2>
            ...
            {config.primary.action ? (
              <Button
                variant={config.primary.action.variant}
                size={config.primary.action.size}
              >
                {config.primary.action.label}
              </Button>
            ) : null}
          </CardHeader>
          <CardBody className="mt-6 grid gap-4">{config.primary.body}</CardBody>
        </Card>

        <Card>
          <CardHeader>
            <p className="text-sm uppercase tracking-[0.2em] text-ink/40">
              {config.secondary.eyebrow}
            </p>
